...
#######################################
# Project: MA-Thesis
# Clean opustools (bilingual) data 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Clean aligned text data from opustools.")
    parser.add_argument("-i","--input_dir", type=str, help="Directory containing files with text content.")
    parser.add_argument("-o","--output_dir", type=str, help="Directory to save the cleaned output files.")
    parser.add_argument("-s","--src_lang", type=str, help="Source language code.")
    parser.add_argument("-t","--trg_lang", type=str, help="Target language code.")
    parser.add_argument("-d","--data_quality", type=str, help="Quality of data cleaning.")
    parser.add_argument("-c","--cosine_filtering", type=str, help="Flag for enabling cosine filtering.")
    parser.add_argument("-a","--all_data_in_one", type=str, help="Flag for enabling writing an output file with all text.") # TODO


    args = parser.parse_args()

    # TODO: Input arguments for more versatility
    src_l = args.src_lang # 'bar'
    trg_l = args.trg_lang # 'de'
    data_quality=args.data_quality
    
    # NOTE: Check if this is needed later?     date_of_experiment = '20240524'
    #input_dir = f'/media/AllBlue/LanguageData/CLEAN/opustools/{src_l}-{trg_l}'
    #output_dir = f'/media/AllBlue/LanguageData/PREP/opustools/{src_l}-{trg_l}/{data_quality}'
    input_dir = args.input_dir
    output_dir = args.output_dir
    excluded_dir = f'{output_dir}/excluded'
    ...
    dir_maker(excluded_dir)

    # Get all files based on language
    src_files = glob.glob(f'{input_dir}/*.{src_l}')
    trg_files = glob.glob(f'{input_dir}/*.{trg_l}')
    ...
    for src_file, trg_file in zip(src_files_reordered, trg_files_reordered):

        src_outfile = os.path.basename(src_file)
        trg_outfile = os.path.basename(trg_file)
        # TODO: Process line-by-line
        # NOTE: Is this even possible/reasonable considering the use of pandas' dataframes?
        ...
        # Extract (and remove) rows where source-side and target-side are the same
        ...
        # TODO: Fix whatever happened here in the past- if necessary?
        ...
        with open(f'{excluded_dir}/{src_outfile}-other','w') as file:
            for line in df_other[src_l]:
                file.write(line + "\n")
        with open(f'{excluded_dir}/{trg_outfile}-other','w') as file:
            for line in df_other[trg_l]:
                file.write(line + "\n")

        with open(f'{excluded_dir}/{src_outfile}-ratio','w') as file:
            for line in df_ratio[src_l]:
                file.write(line + "\n")
        with open(f'{excluded_dir}/{trg_outfile}-ratio','w') as file:
        ...
        # Only apply cosinge filtering if enabled via flag (better results for more similar/related languages)
        if args.cosine_filtering == "True":
            print(f'Applying cosine-filtering on data')
        ...
        # Save current data to output files (source and target)
        with open(f'{output_dir}/{src_outfile}','w') as file:
            for line in df[src_l]:
                file.write(line + "\n")
        with open(f'{output_dir}/{trg_outfile}','w') as file:
        ...
        # """ Output data file containing everything """
        # # Save all data to a single file for training that does not build on cross-validaton
        # with open(f'{output_dir}/{src_l}-all','w') as file:
        #     for line in df[src_l]:
        #         file.write(line + "\n")
        # with open(f'{output_dir}/{trg_l}-all','w') as file:
        #     for line in df[trg_l]:
        #         file.write(line + "\n")
        # # Randomize order
        # df = df.sample(frac = 1).reset_index(drop = True)
        ...
        """ Output data files for cross-validation """
        # TODO: Introduce input parameter to toggle this output variant
        ...
        #     data_splits[index]
        #     with open(f'{output_dir}/{src_l}-{split_string}','w') as file:
        #         for line in data_splits[index][src_l]:
        #             file.write(line + "\n")
        #     with open(f'{output_dir}/{trg_l}-{split_string}','w') as file:
